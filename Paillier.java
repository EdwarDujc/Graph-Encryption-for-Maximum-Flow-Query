...
public class Paillier {
	...
	// bit_length is set as half of k1 so that when pq is computed, the result has k1 bits
	int bit_length = sk.k1/2;

	// Chooses a random prime of length k2. The probability that p is not prime is at most 2^(-k2)
    ...
    public static BigInteger encrypt(BigInteger plaintext, PublicKey pk) {
    	BigInteger randomness = new BigInteger(pk.k1, rnd);  // creates randomness of length k1
...
//    public static BigInteger encrypt(BigInteger plaintext, PublicKey pk) {
//    	BigInteger randomness = new BigInteger(pk.k1, rnd);  // creates randomness of length k1
//    	if(plaintext.compareTo(Zero)==-1){
//    		plaintext=plaintext.negate();
//    		BigInteger tmp1 = plaintext.multiply(pk.n).add(BigInteger.ONE).mod(pk.modulous);
//        	BigInteger tmp2 = randomness.modPow(pk.n, pk.modulous);
//        	BigInteger ciphertext = tmp1.multiply(tmp2).mod(pk.modulous);
//        	return Paillier.encrypt(Zero, pk).subtract(ciphertext);
//    	}
//    	else{
//    		BigInteger tmp1 = plaintext.multiply(pk.n).add(BigInteger.ONE).mod(pk.modulous);
//    		BigInteger tmp2 = randomness.modPow(pk.n, pk.modulous);
...
//       }
	...
	BigInteger plaintext = L(ciphertext.modPow(sk.lambda, sk.modulous), sk.n).multiply(sk.mu).mod(sk.n);
	//return plaintext;
	return ciphertext;
    }
	...
	System.out.print("Ciphertext contains " + content.length + " bytes of the following plaintext:");
    ...
    public static void main(String []args){
	    ...
	    BigInteger plaintext=BigInteger.valueOf(pl);
	    BigInteger cm=Paillier.encrypt(plaintext, pk);
		for(int i=0;i<10000;i++){
			
			BigInteger dm=Paillier.decrypt(cm, sk);
		}
		long end=(System.currentTimeMillis()-begin)/10000;
